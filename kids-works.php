...
// 子供向け作品テーブルから全件取得（即時公開のため審査なし）
$whereClause = "";
$params = [];

// 総件数を取得
$countSql = "SELECT COUNT(*) FROM kids_artworks";
...
// データを取得
$sql = "SELECT * FROM kids_artworks ORDER BY is_featured DESC, created_at DESC LIMIT ? OFFSET ?";
$params = [$perPage, $offset];
$stmt = $pdo->prepare($sql);
$stmt->execute($params);
$artworks = $stmt->fetchAll();

// AIストーリーがある作品をランダムに3件取得
$kindStories = [];
if (!empty($artworks)) {
    // AIストーリーがある作品をフィルタリング
    $storyArtworks = array_filter($artworks, function($artwork) {
        return !empty($artwork['ai_story']);
    });
    
    // ランダムに並び替えて最大3件取得
    if (!empty($storyArtworks)) {
        $storyArtworks = array_values($storyArtworks);
        shuffle($storyArtworks);
        $kindStories = array_slice($storyArtworks, 0, 3);
    }
}
?>
...
<head>
    ...
    <meta name="description" content="こどもたちが作った素敵な作品を集めたアトリエです。AIが作ったお話も読めます。">
    <link rel="icon" href="/favicon.ico">
    
    <!-- カノニカルタグ -->
    <?php
    $canonicalUrl = ($_SERVER['REQUEST_SCHEME'] ?? 'https') . '://' . $_SERVER['HTTP_HOST'] . '/kids-works.php';
    if ($page > 1) {
    ...
    <div class="container mt-4" id="materials">
        ...
        <div class="row">
            ...
            <div class="col-6 col-md-4 col-lg-3 col-xl-2 col-xxl-2 mb-4">
                ...
                <a href="/kids-work.php?id=<?= $artwork['id'] ?>" 
                   class="card material-card h-100" 
                   role="button" 
                   tabindex="0" 
                   aria-label="<?= h($artwork['title']) ?>の詳細を見る">
                    <?php
                        // パスの正規化（先頭の/を除去してから/を付与）
                        $imagePath = $artwork['webp_path'] ?: $artwork['image_path'];
        ...
        <section class="kind-stories-section mt-5 mb-5">
            ...
            <div class="kind-stories-list">
                ...
                <div class="kind-story-item">
                    <!-- 画像（リンク） -->
                    <a href="/kids-work.php?id=<?= h($story['id']) ?>" class="text-decoration-none">
                        <div class="kind-story-image-wrapper">
                            <?php
                            // パスの正規化
                            $storyImage = !empty($story['webp_path']) 
                                ? $story['webp_path']
                                : $story['image_path'];
                            $storyImagePath = '/' . ltrim($storyImage, '/');
                            ?>
                            <div class="kind-story-image">
                                <img src="<?= h($storyImagePath) ?>" 
                                     alt="<?= h($story['title']) ?>"
                                     loading="lazy"
                                     decoding="async">
                            </div>
